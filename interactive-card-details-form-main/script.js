...
inputs.forEach((input)=>{
  input.addEventListener('input' , (event) =>{
    ...
    const errMsgDisplay = event.target.parentElement.getElementsByClassName('err-msg');
...
const processInput = {
  'cardholder-name' : (value) =>{
    return {valid : testRegEx(/^([a-z]{2,})\s([a-z]{2,})$/i , value) , data : value , err : 'Wrong format, letters only , two names required'}
  },
  'card-number' : (value)=>{
    return {valid : testRegEx(/^((\s)*(\d{4})(\s)*){4}$/ , value) , data : treatNumberFormat(value) , err : 'Wrong format, numbers only'}
  },
  'exp-date-month' : (value)=>{
    const intValue = parseInt(value , 10);
    const displayValue = value.slice(0 , 2);

    return {valid : testRegEx(/^(\d){2}$/ , value) && inRange(intValue , 1 , 12), data : displayValue , err : 'Wrong format, numbers only'}
  },
  'exp-date-year' : (value)=>{
    const intValue = parseInt(value , 10);
    const displayValue = value.slice(0 , 2);

    return {valid : testRegEx(/^(\d){2}$/ , value) && inRange(intValue , 0 , 99), data : displayValue , err : 'Wrong format, numbers only'}
  },
  'cvc' : (value)=>{
    const displayValue = value.slice(0,3);
    return {valid : testRegEx(/^(\d){3}$/ , value) , data : displayValue , err : 'Wrong format, numbers only'}
  }
};
